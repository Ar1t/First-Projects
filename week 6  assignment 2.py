...
def make_str_from_column(board, column_index):
    ...
    column_string = ''
    for i in board:
        column_string = column_string + i[column_index]
...
def board_contains_word(board, word):
    ...
    for column_index in range(len(board)):
        for row_index in range(len(board)):
            if word in make_str_from_column(board, column_index) or make_str_from_row(board, row_index):
                return True
...
def update_score(player_info, word):
    ...
    """
    old_score = 0
    player_info = ['input(enter name)',old_score]
    new_score = word_score(word)
    return player_info.append(new_score + old_score)
...
def read_words(words_file):
    ...
    """
    wordlist1 = [item.rstrip() for item in words_file]
    return wordlist1
